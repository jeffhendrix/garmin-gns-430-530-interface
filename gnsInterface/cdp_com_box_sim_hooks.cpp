...
static unsigned long __cdecl my_reg_read(unsigned long num, unsigned long *addr, unsigned long size, unsigned long p4)
{
	...
	if(0x7f0 == num)
	{
		pIntf->com1_active = *(unsigned long*)addr / 10;
		pIntf->com1_changed = true;
	}else if(0x7f4 == num)
	{
		pIntf->com1_standby = *(unsigned long*)addr /10;
		pIntf->com1_changed = true;
	}
...
static unsigned long __cdecl my_SYS_pvg_var_ctrl(unsigned long p1, unsigned long p2)
{
	...
	if(first_time)
	{
		...
		freq = pIntf->com1_active*10;
		g_reg_write(0x7f0, &freq, 4,0);
		freq = pIntf->com1_standby*10;
		g_reg_write(0x7f4, &freq, 4,0);
...
int hook_cdp_com_box_sim(GNSIntf* pSharedInterface)
{
	int res = 0;
	int i;
